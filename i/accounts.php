<?php
    /**
     * @param mysqli|null $connection The database connection to use, or null to
     * create a new one
     * @param string $email The email address to search for
     * @param string $password The password to search for
     * @return int|null|string The account's ID or null if no account was found,
    ...
    function fetch_login_account_id(
        ?mysqli $connection,
        string $email,
        #[SensitiveParameter] string $password): int|null
    {
        if ($connection === null)
        {
            $connection = mysqli_connect("localhost", "root", "", "advweb");
            if (!$connection)
                throw new LogicException(mysqli_connect_error());
        }
     ...
     * @return int|string The new account's ID, or an error message if failed
     * @throws mysqli_sql_exception
     */
