import type { ServerJSONShapeMap } from "../server_types.js";

let previousSearchCancel: AbortController | null = null;

const forMoviesForm = document.getElementById("search-for-movies") as HTMLFormElement;
const searchResultsContainer = document.getElementById("search-results") as HTMLDivElement;

let starFilledIcon: string | null = null;
let starIcon: string | null = null;
...
function updateAsMovies(movies: Iterable<
    {
        id: number,
        title: string,
...
function requestUpdateAsMovies()
{
    ...
    const timeout = setTimeout(
        async () =>
        {
            ...
            const url = new URL(
                `${location.origin}${location.pathname.replace(/\/[^\/]+$/, "")}/json/search_media.php`);
            ...
            const response = await (
                ...
                .json() as ServerJSONShapeMap["/json/search_media.php"];
